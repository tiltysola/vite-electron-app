...
/* LoggerFun: start */
let logger: fs.WriteStream | null = null;
...
const createLogger = () => {
  if (logger) {
    try {
      ...
      console.error('Error closing existing logger:', error);
    }
  }
  logger = fs.createWriteStream(path.join(logPath, logFileName));
...
const log = (level: string, color: keyof typeof chalk, ...msg: string[]) => {
  ...
  console.log(`${chalk.grey(`[${now}]`)} ${(chalk as any)[color](`[${level}]`)} ${msg.join(' ')}`);
};
...
stopHandler.push(() => {
  ...
  if (logger) {
    try {
      ...
      console.error('Error closing logger:', _error);
    }
  }
});
/* LoggerInit: end */
